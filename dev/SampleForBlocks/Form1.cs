...
// ReSharper restore InconsistentNaming
    {
        ...
        private void CopyDataToSecondTable()
        {
            for (int i = 0; i < m_fmBlock.Parameters.Count; ++i)
            {
                ...
                fmDataGrid2.Rows[i].Cells[2].Value = m_fmBlock.Parameters[i].globalParameter.defaultXRange.MinValue / coef;
                fmDataGrid2.Rows[i].Cells[3].Value = m_fmBlock.Parameters[i].ValueInUnits;
                fmDataGrid2.Rows[i].Cells[4].Value = m_fmBlock.Parameters[i].globalParameter.defaultXRange.MaxValue / coef;
                Color colorMin = m_fmBlock.Parameters[i].value.defined == false
                    || m_fmBlock.Parameters[i].value.value < m_fmBlock.Parameters[i].globalParameter.defaultXRange.MinValue
                        ? Color.Pink
                        : Color.White;
                Color colorMax = m_fmBlock.Parameters[i].value.defined == false
                    || m_fmBlock.Parameters[i].value.value > m_fmBlock.Parameters[i].globalParameter.defaultXRange.MaxValue
                        ? Color.Pink
                        : Color.White;
